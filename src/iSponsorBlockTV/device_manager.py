...
class DeviceManager:
    ...
    async def __call__(self, state: Any) -> None:
        ...
        self.main_task = self.api_helper.create_task(self.process_playstatus(state, time_start))
    ...
    async def loop(self) -> None:
        ...
        while not self.cancelled:
           ...
           #             await self.start()
            ...
            self.logger.info(
                "Connected to device %s (%s)", lounge_controller.screen_name, self.device.name
            )
            try:
    ...
    async def process_playstatus(self, state: Any, time_start: Any) -> None:
        ...
        if state.state.value == 1:  # Playing
            ...
            if segments:  # If there are segments
                await self.time_to_segment(segments, state.currentTime, time_start)  # Skip to the next segment


    # Finds the next segment to skip to and skips to it
    async def time_to_segment(self, segments: list[Any], position: int, time_start: float) -> None:
        ...
        if start_next_segment:
            time_to_next = (
                start_next_segment - position - (time.time() - time_start) - self.device.offset
            )
    ...
    async def skip(self, time_to, position, uuids):
        ...
        await self.api_helper.create_task(self.api_helper.mark_viewed_segments(uuids))
        await self.api_helper.create_task(self.controller.seek_to(position))
